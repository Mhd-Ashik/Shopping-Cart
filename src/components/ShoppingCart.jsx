  ...
  return (
    ...
    {cartItems.length !==0 && (
      ...
      <div className='w-full h-16 bg-white absolute left-0 top-0 z-10 grid place-items-center border rounded-lg'>
          ...
          <button 
          className='w-9 h-9 bg-yellow-400 absolute right-3 z-20 grid place-items-center border-2 rounded-full hover:bg-yellow-500 transition-colors' onClick={() => setIsOpen(false)}>
      ...
      <button className='w-9 h-9 bg-yellow-400 absolute -left-14 top-3 z-20 grid place-items-center border-2 rounded-full'onClick={() => setIsOpen(true)}>
          ...
          <span 
          className='w-6 h-6 bg-pink-400 absolute -bottom-4 -left-2 grid place-items-center border border-gray-300 rounded-full text-sm text-white'>
